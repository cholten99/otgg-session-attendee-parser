...
# PART TWO : Initialise the Gmail API
...
# PART FIVE : Read in attendee CSV
...
# PART SIX : Read in session attendance CSV
...
# PART SEVEN : Loop through sessions and locations
for session_row in session_docs_list:

  # PART EIGHT : For each new pairing map into the template email body
  session = "Session " + session_row[0]
  location = session_row[1]
  doc = session_row[2]

  # PART NINE : Create a list of email addresses for people who went to session
  email_to_list = []
  ...
  # PART TEN : Map info into email template
  ...
  # PART ELEVEN : Use list of email addresses to send to everyone
  message = MIMEText(message_body)
  subject = "One Team Gov Global follow-up : " + location + " " + session.lower()
  message['subject'] = subject

  # PART TWELVE : Set up to and from
  ...
  # PART THIRTEEN : Skip if no-one is recorded as going
  if (to_list == ""):
    continue
  else:
    message['to'] = to_list

  # PART FOURTEEN : Send it off
